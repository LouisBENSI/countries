<template>
  ...
  <main class="max-w-[1280px] px-4 mx-auto xl:px-0 dark:bg-gray-800">
    ...
    <div class="flex flex-col justify-between my-12 md:flex-row">
      ...
      <select v-model="selected" class="mt-1 block w-full pl-3 py-2 text-base border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm md:mt-0 md:ml-2 md:w-[300px]">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
      <!-- COUNTRY -->
      <div v-for="country in countries" :key="country.id" class="w-full rounded-md overflow-hidden shadow border border-solid border-gray-200">
        <!-- IMG -->
